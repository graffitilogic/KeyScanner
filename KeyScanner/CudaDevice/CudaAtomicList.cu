...
cudaError_t CudaAtomicList::init(unsigned int itemSize, unsigned int maxItems)
{
	...
	*_countHostPtr = 0;
	...
	if (err) {
		...
		if (_countHostPtr != NULL) cudaFreeHost(_countHostPtr);
		if (_countDevPtr != NULL) cudaFree(_countDevPtr);
		if (_hostPtr != NULL) cudaFreeHost(_hostPtr);
		if (_devPtr != NULL) cudaFree(_devPtr);
