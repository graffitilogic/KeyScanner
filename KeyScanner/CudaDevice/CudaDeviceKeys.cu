...
{
	// force erase and free
	//cudaError_t err = cudaMemset(_devX, 0, sizeof(unsigned int) * _count * 8);
	//cudaError_t err = cudaMemcpy(_devX, 0, sizeof(unsigned int) * _count * 8, cudaMemcpyHostToDevice);
	cudaFree(_devX);

	//cudaError_t err = cudaMemset(_devY, 0, sizeof(unsigned int) * _count * 8);
	//err = cudaMemcpy(_devY, 0, sizeof(unsigned int) * _count * 8, cudaMemcpyHostToDevice);
	cudaFree(_devY);
...
{
	//cudaError_t err = cudaMemset(_devBasePointX, 0, _count * 8 * sizeof(unsigned int));
	//cudaError_t err = cudaMemcpy(_devBasePointX, 0, _count * 8 * sizeof(unsigned int), cudaMemcpyHostToDevice);
	cudaFree(_devBasePointX);

	//err = cudaMemset(_devBasePointY, 0, _count * 8 * sizeof(unsigned int));
	//err = cudaMemcpy(_devBasePointY, 0, _count * 8 * sizeof(unsigned int), cudaMemcpyHostToDevice);
	cudaFree(_devBasePointY);

	//cudaError_t err = cudaMemset(_devPrivate, 0, sizeof(unsigned int) * _count * 8);
	//err = cudaMemcpy(_devPrivate, 0, sizeof(unsigned int) * _count * 8, cudaMemcpyHostToDevice);
	cudaFree(_devPrivate);

	//err = cudaMemset(_devChain, 0, _count * sizeof(unsigned int) * 8);
	//err = cudaMemcpy(_devChain, 0, _count * sizeof(unsigned int) * 8, cudaMemcpyHostToDevice);
	cudaFree(_devChain);
...
void CudaDeviceKeys::clearProgress()
{
	_step = 0;
	_count = 0;
}
