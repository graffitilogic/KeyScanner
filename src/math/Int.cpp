...
void Int::ZeroLeft(int len) {
	Int b(this);

	b.ZeroRight(b.GetBase16Length() - len);
	Sub(&b);
}
...
int Int::GetBase16Length() {
	//accuracy issues:
	//return GetBitLength() / 4;
	return GetBase16().length();
}
...
std::vector<std::string> Int::GetChunks(std::vector<Int> values) {
	std::vector<std::string> results;
	for (int i = 0; i < values.size(); i++) {
		results.push_back(values[i].GetBase16());
	}
	return results;
}

std::vector<Int> Int::GetChunks(int chunkSize = 1) {
	//std::vector<std::string> strResults;
	std::vector<Int> interimResults;
	std::vector<Int> finalResults;

	Int divSixteen = Int::sixteen();
	...
	//std::string srcKeyStr = src.GetBase16().c_str();
	Int keyPart = src;
	while (!bFinished) {
		Int rem;
		keyPart.Div(&divSixteen, &rem);
		//std::string keyPartStr = keyPart.GetBase16().c_str();
		//std::string remStr = rem.GetBase16().c_str();
		if (keyPart.IsGreater(&zero)) {
			interimResults.push_back(rem);
			//strResults.push_back(rem.GetBase16().c_str());
		}
		else {
			if (rem.IsGreater(&zero)) {
				interimResults.push_back(rem);
				//strResults.push_back(rem.GetBase16().c_str());
			}
			bFinished = true;
	...
	if (chunkSize > 1) {
		std::reverse(interimResults.begin(), interimResults.end());

		Int partBuffer = interimResults[0];
		int chunkLen = 1;
		for (int i = 1; i < interimResults.size(); i++) {
			if (chunkLen == chunkSize) {
				finalResults.push_back(partBuffer);
				partBuffer = interimResults[i];
				chunkLen = 1;
			}
			else {
				partBuffer.Mult(16);
				partBuffer.Add(&interimResults[i]);
				chunkLen++;
			}
		}
		finalResults.push_back(partBuffer);
...
void Int::DeriveStartKey(int chunkSize) {
	Int result;
	result.SetInt64(1);
...
void Int::DeriveEndKey(int chunkSize) {
	Int result;
	result.SetBase16("f");
...
Int Int::GetChunkDistance(int chunkSize) {
	
	Int result;

	Int start;
	start.DeriveStartKey(chunkSize);

	Int end;
	end.DeriveEndKey(chunkSize);
...
void Int::SetChunks(std::vector<Int> chunks) {
	Int partBuffer;
	partBuffer.SetInt64(0);

	//std::reverse(chunks.begin(), chunks.end());
	
	for (int i = 0;i < chunks.size();i++) {
		Int chunk = chunks[i];
		int chunkLen = chunk.GetBase16().length();
		for (int j = 0; j < chunkLen; j++) {
			partBuffer.Mult(16);
		}
		partBuffer.Add(&chunk);
	}

	Set(&partBuffer);
}

void Int::SetStrChunks(std::vector<std::string> chunks) {
	...
	SetBase16(strResult.c_str());
}

void Int::SetStrChunks(std::vector<std::string> chunks, int keyLen) {
	Int partBuffer;
	partBuffer.SetStrChunks(chunks);

	bool isSmaller = (partBuffer.GetBase16().size() < keyLen);
	//determine the nominal chunk size
	int NOMINAL_CHUNK_SIZE = 0;
	for (int s = 0; s < chunks.size(); s++) {
		if (NOMINAL_CHUNK_SIZE < chunks[s].size()) NOMINAL_CHUNK_SIZE = chunks[s].size();
	}

	/*
	if (isSmaller) {
		OutputDebugString("Smaller");
		OutputDebugString("\n");
	}
	else {
		OutputDebugString("Not Smaller");
		OutputDebugString("\n");
	}
	*/

	for (int s = 1; s < chunks.size() - 1; s++) {		//skip last, probably should skip first too?  I only really want to modify the center chunks.  1st chunk is targeting a range, last chunk will get fast-scanned by the gpu
		if (isSmaller) {
				while (chunks[s].size() < NOMINAL_CHUNK_SIZE) {
					chunks[s] = "0" + chunks[s];
				}
			}
		else {
				//this assumes the chunk will only ever be 1 hex larger than needed, which in this case is true but for utility //TODO: refactor
				while (chunks[s].size() > (NOMINAL_CHUNK_SIZE - 1)) {
					chunks[s] = chunks[s].substr(1, chunks[s].size() - 1);
			}
		}
	}

	Int testLen;
	testLen.SetStrChunks(chunks);

	//fix issues with last chunk that probably don't matter anyway because of later Pad-Zeroes-Right stuff
	while (testLen.GetBase16().size() < keyLen) {
		chunks[chunks.size() - 1] = "0" + chunks[chunks.size() - 1];
		testLen.SetStrChunks(chunks);
	}
	...
	//OutputDebugString(GetBase16().c_str());
	//OutputDebugString("\n");

}

void Int::SetChunks(std::vector<Int> chunks, int keyLen) {
	...
	//does pure math version work for this or do we need to do string hackery?
	if (partBuffer.GetBase16().size() == keyLen) {
		//OutputDebugString("Math-Based");
		//OutputDebugString("\n");
		Set(&partBuffer);
	}
	else {
		std::vector<std::string> workingVector = partBuffer.GetChunks(chunks);
		SetStrChunks(workingVector, keyLen);
		//OutputDebugString("String-Based");
		//OutputDebugString("\n");
	}
}

std::vector<Int> Int::Derive(Int* a){
	/*
	int count = 0;
	Int* results = new Int[count];
	*/
	std::vector<Int> results;
	std::vector<int> permutation_helper;
	std::vector<std::string> key_segments;

	std::string sKey = a->GetBase16().c_str();
	std::string r_sKey = sKey;
	
	reverse(r_sKey.begin(), r_sKey.end());   // Reverse, so as to prioritize grouping away from the start (ranging purposes)

	int count = 0;
	for (unsigned i = 0; i < r_sKey.length(); i += 8) {
		std::string sTmp = r_sKey.substr(i, 8);
		
		reverse(sTmp.begin(), sTmp.end());
		key_segments.push_back(sTmp);
		permutation_helper.push_back(count);

		count++;
	}

	permutation_helper.pop_back(); //remove the last, again - ranging purposes.
	std::sort(permutation_helper.begin(), permutation_helper.end());  //next_permutation from stl algorithm want to be sorted.

	//permute the indexes - build keys;
	std::string keyPrefix = key_segments.back();

	do {
		std::string thisKey = keyPrefix;

		for (int p : permutation_helper)
		{
			thisKey.append(key_segments[p]);
		}

		Int key;
		key.SetBase16(thisKey.c_str());
		results.push_back(key);
	} while (std::next_permutation(permutation_helper.begin(), permutation_helper.end()));
	return results;
}

uint64_t Int::DeriveRandomizerWidth() {
	std::string sKey = GetBase16();
	uint64_t sKeyLen = sKey.size();

	uint64_t retVal = sKeyLen % 4 == 0 ? sKeyLen / 4 : (sKeyLen / 4) + 1;
...
void Int::Rand3(Int* min, Int* max) {
	CLEAR();
	Int diff;
	int nbit = 256;
	uint32_t nb = nbit / 32;
	...
	for (; i < nb; i++) {
		bits[i] = rndl();
		std::string step_inspection = this->GetBase16();
	}
	this->Mod(&diff);
	this->Add(min);
}
