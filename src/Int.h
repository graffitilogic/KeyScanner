...
public:
	...
	void ExpandRight(int len);
	void ZeroRight(int len);
	void ZeroLeft(int len);

	//what about an asarray(n-n-n-n) for comparison reasons?
	...
	int GetBase16Length();	// Base16 String Length
	...
	void Rand3(Int* min, Int* max);
	void Set32Bytes(unsigned char* bytes);
	void MaskByte(int n);

	std::vector<int> GetPositionMap();

	std::vector<Int> GetChunks(int chunkSize);
	std::vector<std::string> GetChunks(std::vector<Int> values);
	std::vector<Int> Derive(Int* a);

	uint64_t DeriveRandomizerWidth();

	void DeriveStartKey(int chunkSize);
	void DeriveEndKey(int chunkSize);

	Int GetChunkDistance(int chunkSize);

	void SetChunks(std::vector<Int> chunks);
	void SetStrChunks(std::vector<std::string> chunks);
	void SetStrChunks(std::vector<std::string> chunks, int keyLen);
	void SetChunks(std::vector<Int> chunks, int keyLen);
