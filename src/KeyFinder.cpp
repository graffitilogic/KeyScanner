...
std::vector<Int> KeyFinder::getOpenSSLRandoms(Int min, Int max, uint64_t length, uint64_t rKeyPer) {
	//uses libsecp256k1 with some help of openSSL, doesn't sort the net, uses max-min as spacing
	Int rKeyMask;
	rKeyMask.SetInt64(rKeyPer);
	...
	//if (rndGPU.getRandomizersSize() == 0) 	rndGPU.loadRandomizers(timeSeed, NET_SIZE);
	//if (true) rndGPU.loadRandomizers(timeSeed, NET_SIZE);
	...
	vector<std::string> strResults = OpenSSLRNG::getRandoms(min.GetBase16(), max.GetBase16(), NET_SIZE);// = rndGPU.getRandomPool(min, max, NET_SIZE);
	for (uint64_t k = 0; k < NET_SIZE; k++) {
		Int newKey;
		newKey.SetBase16(strResults[k].c_str());
		preResults.push_back(newKey);
	}
...
std::vector<Int> KeyFinder::getOpenSSLRandoms2(Int min, Int max, uint64_t length, uint64_t rKeyPer) {
	//uses libsecp256k1 with some help of openSSL, sorts the net, uses middle distance division as spacing
	Int rKeyMask;
	rKeyMask.SetInt64(rKeyPer);
...
std::vector<Int> KeyFinder::getOpenSSLRandoms3(Int min, Int max, uint64_t length, uint64_t rKeyPer) {
	//uses libsecp256k1, sorts the net, uses chunked(len) avg as spacing
	...
	std::sort(preResults.begin(), preResults.end(), custom_comparer); //for distribution analysis but not necessary
	...
	for (int i = 0; i < preResults.size(); i++) {
		...
		//OutputDebugString(adjustedKey.GetBase16().c_str());
		//OutputDebugString(" -> ");
		...
		for (int m = 0; m < middleKeyCount - 1; m++) {
			...
			/*
			if (derivedKey.GetBase16().size() < KEY_LEN) {
				std::string wtf = derivedKey.GetBase16();
			}*/

			//OutputDebugString(derivedKey.GetBase16().c_str());
			//OutputDebugString("\n");
...
std::vector<Int> KeyFinder::getOpenSSLRandoms4(Int min, Int max, uint64_t length, uint64_t rKeyPer) {
	//uses libsec256k1, sorts the net, uses chunked middle distance division as spacing
	Int rKeyMask;
	rKeyMask.SetInt64(rKeyPer);
...
void KeyFinder::getEvenlyDistributedGPUStartingKeys(Random::GPURand& rndGPU, Int& tRangeStart, Int& tRangeEnd, int groupSize, int nbThread, Int* keys, Point* p)
{
	...
	if (rKey > 0) {
		...
		if (rangeDiff2.GetBitLength() > 1) {
			...
			std::vector<Int> initial_Randoms = getOpenSSLRandoms3(rangeStart, rangeEnd, nbThread, rKeyPer);// getGPURandomsED1(rndGPU, rangeStart, rangeEnd, nbThread, rKeyPer);
